"""
A2_8 — Monte Carlo uncertainty driver for the PBS proton plan.

Re-runs the optimised PBS plan produced by ``pbs_proton.py`` with
...
Outputs (under ``A2_8/output/uncertainty/``):
...
Usage:
    python3 A2_8/run_uncertainty.py
    python3 A2_8/run_uncertainty.py --force
    python3 A2_8/run_uncertainty.py --rescore
"""
...
TEMPLATE_PATH = os.path.join(SCRIPT_DIR, "output", "dose_pbs_patient.txt")
UNCERT_DIR    = os.path.join(SCRIPT_DIR, "output", "uncertainty")
...
def replicate_basename_rel(k):
    return os.path.join("A2_8", "output", "uncertainty", f"replicate_{k:02d}")
...
def main():
    ...
    print("=" * 78)
    print("A2_8 — PBS PROTON MONTE CARLO UNCERTAINTY")
    print("=" * 78)
    ...
    if not os.path.isfile(TEMPLATE_PATH):
        print("Template not found. Run A2_8/pbs_proton.py first.")
        return
    ...
    print("\nDone.")
    print("Next: python3 A2_8/analyse_dvh_uncertainty.py")
